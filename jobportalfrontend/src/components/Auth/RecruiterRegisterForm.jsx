...
import api from "../../api/axiosConfig"; // Ensure this has the correct baseURL
...
const RecruiterRegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const submissionData = { name, email, password, company, location };
    ...
    try {
      ...
      const response = await api.post("/api/v1/signup", submissionData);
      ...
      if (error.response) {
        ...
        setError(
          error.response.data?.message ||
            `Signup failed! Status: ${error.response.status}`
        );
